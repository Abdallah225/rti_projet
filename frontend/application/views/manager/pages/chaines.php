				...
				<div class="card-body">
					...
					<div class="card">
						<div class="card-header ch-alt">
							<h2><small class="titre">Formulaire de création des chaines</small></h2>
						</div>
						...
						<div class="card-body card-padding">
							...
							<div class="row">
								<div class="col-md-6">
									...
									<div class="form-group fg-float m-b-30">
										<div class="fg-line">
											<input type="text" class="form-control input-sm" id="sai_streaming">
											<label class="fg-label">Url streaming</label>
										</div>
									</div>
					...
					<div class="table-responsive">
						<table id="data-table-command" class="table table-striped">
							...
							<tr>
								...
								<th data-column-id="url" data-order="desc">Url TV</th>
	...
	<script type="text/javascript">
		...
		function vider_champ()
		{
			...
			$('#sai_streaming').val('');
			$('#sai_description').val('');
			$('.titre').text('Formulaire de création de chaines')
		...
		$("#data-table-command").bootgrid({
			...
			formatters: {
				"commands": function(column, row) {
					return "<button type=\"button\" onclick=\"fupdate('"+row.id+"','"+row.libelle+"','"+row.url+"','"+row.desc+"')\" class=\"btn btn-icon command-edit waves-effect waves-circle\" data-row-id=\"" + row.id + "\"><span class=\"zmdi zmdi-edit\"></span></button> " +
						"<button type=\"button\" onclick=\"fdelete('"+row.id+"','"+row.libelle+"')\" class=\"btn btn-icon command-delete waves-effect waves-circle\" data-row-id=\"" + row.id + "\" data-libelle=\""+row.libelle+"\"><span class=\"zmdi zmdi-delete\"></span></button>"
				}
			}
		});

		function fupdate(id,libelle,url,desc) {
			$('#sai_libelle').val(libelle);
			$('#sai_streaming').val(url);
			$('#sai_description').val(desc);
			$('#i').val(id);
			$('.titre').text('Formulaire de modification de chaines')
		...
		$('#btn_valider').click(function () {
			...
			if(url ==='')
			{
				toast('Merci de saisir l\'url de la chaine','warning');
				return false;
			}
			...
			$.ajax({
				url: '<?php echo site_url(); ?>/Chaine/maj',
				type: 'post',
				dataType: 'json',
				data: JSON.stringify(chaine),
				headers: {
					"session": "<?php echo $this->session->session; ?>"
				},
